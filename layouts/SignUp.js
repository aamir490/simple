...
function SignUp() {
  ...
  const usernameCheck = async () => {
    try {
      const response = await axios.post("/api/auth/signup/username_check", { username1 });
      setResError('green')
    } catch (error) {
      setResError('red')
    }
  ...
  const handleGithubSignin = async (e) => {
    e.preventDefault();
    const status = await signIn('github', { callbackUrl:process.env.REDIRECT_URL_AFTER_SIGN_IN  })
  }
  ...
  async function onSubmit(values) {
    try {
      const response = await axios.post("/api/auth/signup", { values, username1 });
      ...
      <section className={styles.main_page}>
        ...
        <div>
              ...
              <form onSubmit={formik.handleSubmit} autoComplete="off">
                <span style={{ color: 'red', fontSize: '1.5rem', lineHeight: '2.2rem' }} >{result}</span>
                <div className={styles.input_bx}>
                  <input
                    ...
                    style={formik.errors.email && formik.touched.email ? { outline: '1px solid red' } : null}

                  />
                  <span className={styles.label}>Mobile Number or Email</span>
                </div>
                ...
                <div className={styles.input_bx}>
                  <input
                    ...
                    style={formik.errors.fullname && formik.touched.fullname ? { outline: '1px solid red' } : null}

                  />
                  <span className={styles.label}>Full Name (First & Last Name)</span>
                </div>
                ...
                <span className={styles.passInput_icon}>
                  <div className={styles.input_bx}>
                    <input
                      ...
                      style={formik.errors.password && formik.touched.password ? { outline: '1px solid red' } : null}
                ...
                <div className={styles.minusAgree}>
                  <p className={styles.agree} >
                    By signing up, you agree to the Famstep
                  
                    <Link href="/">
                    <span className={styles.my_link} style={{marginLeft:'.5rem'}}> User Agreement, </span>
                    </Link>
